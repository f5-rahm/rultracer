...
**Deploy loop (UDF box):** `./build/build-rpm.sh 0.5.0 0001` ; `scp -O -P 47001 build/dist/rultracer-0.5.0-0001.noarch.rpm root@<udf-host>:/shared/images/` ; `ssh -p 47001 root@<udf-host> /shared/images/install-onbox.sh 0.5.0-0001` ; then hard-refresh. Bump the release number each on-box iteration. New presentation JS files must be added to `build-rpm.sh` staging + `%files`.
