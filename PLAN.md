...
3. **Multi-TMM** — **partially resolved 2026-06-17 on a 4-TMM VE** (ground-truth capture at `background info/raw_capture.txt`); full build is Phase 6. Findings: the 4 logical TMMs are **threads of one process** (shared pid 11313), so the syslog prefix stays bare `tmm[pid]` and the **partition key is field 5 (`ctxId`) = the per-TMM worker-thread id** (capture shows 2 distinct values, `11313`/`11674`; earlier single-TMM VE had `ctxId 22623 ≠ pid 22555`, i.e. a thread id, not the pid and not a 0–3 index). **A flow pins to one TMM** (no flowId spanned two `ctxId`s) → key the partition by `(ctxId, flowId)`. The parser already carries `record.ctxId` (`parser.js:94`). **Still deferred to real multi-blade hardware** (separate `tmm` processes): whether the prefix becomes `tmmN[pid]`, whether a trailing 13th CSV field appears, and the per-TMM start "alert" text.
